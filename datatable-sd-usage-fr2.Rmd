...
1. the function^{*} `patterns()` to filter *column names* by regular expression
1. integer and logical vectors
...
1. The LHS, `names(.SD)`, indicates which columns we are updating - in this case we update the entire `.SD`.
1. The RHS, `lapply()`, loops through each column of the `.SD` and converts the column to a factor.
1. We use the `.SDcols` to only select columns that have pattern of `teamID`.
...
```{rsd_for_lm, cache = FALSE, fig.cap="Fit OLS coefficient on W, various specifications, depicted as bars with distinct colors."}
