...
1. Définir une `data.table` donnant le pourcentage des taxes à appliquer aux
...
1. Définir une `data.table` simulant les produits reçus chaque lundi avec des
...
1. Définir une `data.table` pour afficher des promotions certains jours de la
...
## 2. Syntaxe le la jointure `data.table`
...
The next diagram shows a description for each basic argument. In the following
...
- **De un à un**: lorsque chaque valeur sélectionnée est unique dans chaque table.
- **De un à plusieurs** : lorsque certaines valeurs sélectionnées sont répétées
dans une table et à la fois uniques dans l'autre.
- **De plusieurs à plusieurs** : lorsque les valeurs sélectionnées sont répétées
plusieurs fois dans chaque table.

Dans laplupart des exemples suivants nous réaliserons des correspondances *un
...
- *Jointure croisée avec with data.table*:
